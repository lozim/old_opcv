...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	filter2D(copy, copy, CV_8UC3, kernel);//增强对比度



	medianBlur(copy, copy, 3);//中值滤波
	medianBlur(copy, copy, 3);

	//medianBlur(copy, copy, 3);
	//GaussianBlur(copy, copy, Size(3,3),0,0);
	Mat element = getStructuringElement(MORPH_RECT, Size(3, 3));//腐蚀
	...
	//去除面积小的那些裂缝
	/*vector<int> needtoerace;
	for (int i = 0; i < edege_contours.size(); i++)
	{
		if (contourArea(edege_contours[i], false) < 50)
			needtoerace.push_back(i);
	}
	int counter=0;
	for (int i = 0; i < needtoerace.size(); i++)
	{
		edege_contours.erase(edege_contours.begin()-counter+needtoerace[i]);
		counter++;
	}*/
	//去除面积小的那些裂缝
